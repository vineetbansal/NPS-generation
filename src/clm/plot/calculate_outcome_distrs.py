...
def plot_continuous(outcomes, output_dir, value_map):
    for outcome, df in outcomes.items():
        data = {value_map[source]: value['value'].reset_index(drop=True) for source, value in df.groupby('source')}
        sns.kdeplot(
            data=data,
    ...
    for outcome, df in split_outcomes.items():
        for source in sources:
            if df[df['source'] == source].shape[0] == 0:
                ...
                results[value_map[source]].append(sum(df[df['source'] == source]['value'].values))
...
def plot(outcome_dir, output_dir):
    ...
    value_map = {"model": "Generated Molecules", "train": "Known NPSs", "pubchem": "Pubchem"}
